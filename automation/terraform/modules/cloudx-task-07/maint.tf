...
resource "azurerm_linux_web_app" "app_services" {
  ...
  app_settings = {
    ...
    POSTGRES_HOST                         = azurerm_postgresql_flexible_server.postgres_flexible.fqdn
    POSTGRES_PORT                         = "5432"
    POSTGRES_DATABASE                     = azurerm_postgresql_flexible_server_database.petstore_database.name
    POSTGRES_USERNAME                     = var.postgres_flex.login
    POSTGRES_PASSWORD                     = var.postgres_flex.password
  }
